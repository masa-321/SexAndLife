...
class Media6ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    //TableViewの宣言
        ...
        if Auth.auth().currentUser != nil { //新着はAuthがnilでもいいや。
            ...
            articlesRef.observe(.childAdded, with: {snapshot in
                ...
                if let uid = Auth.auth().currentUser?.uid {
                    ...
                    if self.articleArray.count < 14 && articleData.genreName == "ライフプランニング"{ //トップ記事は10記事まで
            ...
            articlesRef.observe(.childChanged, with: { snapshot in
                if let uid = Auth.auth().currentUser?.uid {
                    ...
                    if articleData.genreName == "ライフプランニング"{ //トップ記事は30記事まで
        ...
        if Auth.auth().currentUser != nil { //新着はAuthがnilでもいいや。
            ...
            articlesRef.observe(.childAdded, with: {snapshot in
                ...
                if let uid = Auth.auth().currentUser?.uid {
                    ...
                    if self.articleArray.count < 30 && articleData.genreName == "ライフプランニング"{ //トップ記事は30記事まで
                        self.articleArray.insert(articleData, at: 0)
                    }
