...
//変更箇所リストアップ：class名、ViewControllerの方でmasterpointer追加（済み）、fetch時の条件処理、Auth.auth().currentUser = nil だった時の処理の消去
        ...
        if Auth.auth().currentUser != nil { //新着はAuthがnilでもいいや。
            ...
            articlesRef.observe(.childAdded, with: {snapshot in
                ...
                if let uid = Auth.auth().currentUser?.uid {
                    ...
                    if self.articleArray.count < 30 && articleData.genreName == "体のこと"{ //トップ記事は30記事まで
