...
class Media4ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    //TableViewの宣言
    ...
    func fetchCellViewModell() {
        ...
        if let user = Auth.auth().currentUser {
            let ref = Firestore.firestore().collection("articleData").whereField("genreName", isEqualTo: "健康のこと")
            let uid = user.uid
        ...
        if Auth.auth().currentUser != nil { //新着はAuthがnilでもいいや。
            ...
            articlesRef.observe(.childAdded, with: {snapshot in
                ...
                if let uid = Auth.auth().currentUser?.uid {
                    ...
                    if self.articleArray.count < 30 && articleData.genreName == "健康のこと"{ //トップ記事は30記事まで
            ...
            articlesRef.observe(.childChanged, with: { snapshot in
                if let uid = Auth.auth().currentUser?.uid {
                    ...
                    if articleData.genreName == "健康のこと"{ //トップ記事は30記事まで
